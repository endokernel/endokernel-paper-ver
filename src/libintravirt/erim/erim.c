 ...
 * a) check that it follows the structure of a 
...
static int markReadOnly(char * addr) {
  ...
  return rawcall(mprotect, page, PAGE_SIZE, PROT_READ);
}

// TODO: Yff remove all printing here, which seems to cause illegal syscall after seccomp filter installed
// TODO: Yff also removed markReadOnly call here, not sure if you need add this somewhere else
int erim_memScanRegion(uint32_t untrustedPKRU, char * origstart,
  ...
  while (length > 0) {
    ...
    if (found) {// found a sequence at found
      printf("found WPPKRU at %p\n", origstart + skip + found);
 ...
 * exits on failure
 */
int _erim_init(unsigned long long shmemSize, int flags);
int erim_init(unsigned long long shmemSize, int flags);
asm(
  "nop\n\t"
  ".align 16\n\t"
  ".global erim_init\n\t"
  ".type erim_init,@function\n\t"
  "erim_init:\n\t"
  "endbr64\n\t"
  "call _erim_init\n\t"
  "retq\n\t"
...
int _erim_init(unsigned long long shmemSize, int flags) {
  ...
  */
  ...
  void * mapret = erim_mmap_domain(ERIM_TRUSTED_DOMAIN_IDENT_LOC, 40960,  // 10 pages for trusted stack
				   PROT_READ|PROT_WRITE,
				   MAP_ANONYMOUS | MAP_PRIVATE,
				   -1, 0, IV_CONF);
  if(mapret != MAP_FAILED) {
    ERIM_TRUSTED_DOMAIN_IDENT = IV_CONF;
    ...
    ERIM_DBM("set trusted domain to %d, flags %d, untrusted pkru %llx", 
      ERIM_TRUSTED_DOMAIN_IDENT, ERIM_TRUSTED_FLAGS, (unsigned long long)ERIM_PKRU_VALUE_UNTRUSTED);
  } else {
    ERIM_ERR("DOMAIN IDENT MMAP FAILED");
    return 1;
  }

  if(flags | ERIM_FLAG_SWAP_STACK) {

    void * stack = erim_mmap_domain(ERIM_ISOLATED_STACK_START, 
      ERIM_ISOLATED_STACK_ALLOC_SIZE, PROT_READ | PROT_WRITE, 
      MAP_ANONYMOUS | MAP_PRIVATE | MAP_GROWSDOWN, -1, 0, 
      IV_USER);

    if(stack != ERIM_ISOLATED_STACK_START) {
      ERIM_ERR("STACK alloc failed");
      return 1;
    }
